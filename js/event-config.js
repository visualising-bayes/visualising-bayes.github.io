...
function addNewDefaultEvent(id=null) {
    ...
    numberInput.addEventListener("input", (e) => {
        const total = document.getElementById('input-num-objects').value;
    ...
    function probabilityAdjusted(){
        ...
        if(adjustedNumEvents < currentNumEvents){
            ...
            for(let i = 0; i < diff; i++){
                ...
                const current = document.querySelectorAll(`.object.${eventBase}-object`);
                const randomElement = current[Math.floor(Math.random() * current.length)];
                swapEventTo(randomElement, `${eventToIncrease}-object`);
            ...
            for(let i = 0; i < diff; i++){
                ...
                const toDecrease = document.querySelectorAll(`.object.${eventToDecrease}-object`);
                const randomElement = toDecrease[Math.floor(Math.random() * toDecrease.length)];
                swapEventTo(randomElement, `${eventBase}-object`);
            }
        }
        updateProbabilityInputs();
